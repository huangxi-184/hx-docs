...
- 确保浏览器已安装 [Tampermonkey](https://www.tampermonkey.net/) 或 [Violentmonkey](https://violentmonkey.github.io/)
  扩展。
...
- **手动测试脚本**：直接打开
  `https://juejin.cn/user/center/signin?from=sign_in_menu_bar`，观察控制台日志（F12），确认脚本正常工作。
...
- 可以通过修改任务动作为启动 Edge 的“后台模式”，但需要额外参数（如 `--headless` 或
  `--background`），但此类模式可能不支持扩展脚本运行，建议保持默认。
